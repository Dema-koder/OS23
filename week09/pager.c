...
int nfu(struct PTE* page_table){
	int min = page_table[0].count, exit_page = 10000000;
	for (int i = 1; i < pages_num; i++)
		if (page_table[i].count < min)
			if (page_table[i].valid != 0) 
				if (page_table[i].frame != -1) 
					min = page_table[i].count, exit_page = i;
	return exit_page;
}
...
int aging(struct PTE* page_table){
	...
	if (timer == 1){
		for (int i = 0; i < pages_num;i++){
			page_table[i].count = page_table[i].count >> 1;
